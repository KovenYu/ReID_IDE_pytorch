...
def main():
    ...
    source_data = Market('data/{}.mat'.format(args.source), state='train', require_views=False)
    ...
    if args.G_structure == 'Bottleneck':
        generator = BottleneckGenerator(args.is_transfer_net).cuda()
    else:
        generator = LongneckGenerator(args.is_transfer_net).cuda()
    ...
    val_stats = ('acc/r1', 'r1_cat', 'var', 'D_real')
...
def train(source_loader, target_loader, net_s, net_t, generator, discriminator,
    ...
    for i, ((imgs_t, _, _), (imgs_s, labels_s)) in enumerate(joint_loader):
        ...
        meters['D_fake'].update(D_fake, args.batch_size)
        meters['D_real'].update(D_real, args.batch_size)
